...
export function LeadDialog({ leadId, onOpenChange }: Props) {
  ...
  const lead = useQuery(api.crm.getLead, { leadId: leadId ?? undefined }) as Lead | null | undefined;
  const notesHistory = (useQuery(api.crm.listLeadNotes, { leadId: leadId ?? undefined }) as { id: string; note: string; created_at: string; author_name?: string }[] | undefined) ?? [];
  ...
  const profilesData = useQuery(api.crm.listProfiles) as { id: string; name?: string | null; email?: string | null }[] | undefined;
  ...
  const lockedByOther = !!lockInfo.locked_by && lockInfo.locked_by !== user?.id && lockExpiresMs > now;
  ...
  const save = async () => {
    if (!lead || !user) return;
    if (lockedByOther && !isAdmin) { toast.error("This lead is currently being handled by another manager."); return; }
    setSaving(true);
    try {
      await updateLead({
        userId: user.id,
          ...
          <>
            <DialogHeader>
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <DialogDescription className="mt-1 tabular-nums">{lead.phone || "No phone"}</DialogDescription>
                </div>
                <span className={`shrink-0 inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${statusColor(lead.status)}`}>
                  {lead.status}
                </span>
              </div>
            </DialogHeader>
            ...
            {lockedByOther && (
              <div className="rounded-md border border-amber-300 bg-amber-50 text-amber-900 px-3 py-2 text-sm flex items-center gap-2">
                ...
                <span className="tabular-nums text-xs">{Math.floor(remainSec / 60)}:{String(remainSec % 60).padStart(2, "0")}</span>
              </div>
            )}
            {lockedByMe && (
            ...
            {lead.phone && (
              <div className="space-y-2">
                <div className="flex gap-2">
                  <Button asChild className="flex-1 h-12 text-base" disabled={lockedByOther && !isAdmin}>
                    <a
                      href={lockedByOther && !isAdmin ? undefined : `tel:${lead.phone}`}
                      onClick={(e) => { void markCalled(e); }}
            ...
            <div>
              ...
              <Textarea value={newNote} onChange={(e) => setNewNote(e.target.value)} placeholder="What happened on this call?" rows={3} />
            ...
            <div>
              <Label className="flex items-center gap-1.5"><History className="size-3.5" /> Activity timeline</Label>

...
export function ContributionTimeline({ leadId }: Props) {
  const items =
    (useQuery(api.crm.listContributions, { leadId }) as Contribution[] | undefined) ?? [];
  ...
  const name = (id: string) =>
    profiles[id]?.name || profiles[id]?.email?.split("@")[0] || "Manager";
    ...
    <ol className="relative border-l border-border ml-2 space-y-3 max-h-64 overflow-y-auto pr-1">
      {items.map((c) => (
        <li key={c.id} className="ml-4">
          <span
            className={`absolute -left-1.5 mt-1.5 size-3 rounded-full ring-2 ring-background ${contributionTone(c.contribution_type)}`}
          />
